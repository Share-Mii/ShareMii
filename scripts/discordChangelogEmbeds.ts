/**
 * Build Discord webhook embed payloads for ShareMii changelogs.
 */

import { normalizeReleaseTypography } from './releaseTypography';

const SHAREMII_EMBED_COLOR = 0x0088ff; /* --color-accent #0088ff */
...
function fitChunks(raw: string, maxChunks: number): string[] {
  ...
  const maxTail = DISCORD_EMBED_DESCRIPTION_MAX - note.length;
...
export function buildChangelogDiscordEmbeds(
  ...
  function appendDescriptionEmbeds(
    ...
    const reserved =
      embeds.length +
      (reserveTrailingFooterSlot && footer !== undefined && footer.length > 0
        ? 1
        : 0);
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (i === 0) {
        e.title = `${titlePrefix} — v${opts.version}`;
      }
      embeds.push(e);
    }
  }
  ...
  if (mainImg !== undefined && mainImg.length > 0) {
    ...
    if (addText) {
      second.title = `${titlePrefix} — v${opts.version}`;
