...
import { discordIcon, icon } from '@/utils/icon';
...
const DISCORD_ICON = discordIcon('login-modal__oauth-icon');
...
export function openLoginModal(initialMode: ModalMode = 'signin'): () => void {
  ...
  function render(): void {
    ...
    oauth.append(
      ...
      buildOAuthButton('discord', 'Discord', DISCORD_ICON, errorEl),
    );
    ...
    const blocks: HTMLElement[] = [form, divider, oauth, footer];
    const discordUrl = getDiscordInviteUrl();
    if (discordUrl) {
      const discordRow = document.createElement('p');
      discordRow.className = 'login-modal__discord';
      const discordLink = document.createElement('a');
      discordLink.href = discordUrl;
      discordLink.target = '_blank';
      discordLink.rel = 'noopener noreferrer';
      discordLink.className = 'login-modal__discord-link interactive';
      discordLink.textContent = 'Join our Discord community';
      discordRow.appendChild(discordLink);
      blocks.push(discordRow);
    }
...
function buildOAuthButton(
  ...
  btn.type = 'button';
  btn.className = `login-modal__oauth-btn interactive${provider === 'discord' ? ' login-modal__oauth-btn--discord' : ''}`;
